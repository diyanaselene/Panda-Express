...
const SCORE_PER_INCREASE: float = 400.0
const MAX_INCREASES: int        = 5
const MIN_SPAWN_INTERVAL: float = 0.8
const BARREL_SPAWN_HEIGHT: float = 300.0
...
func _ready() -> void:
	...
	was_already_maxed = GameData.training2_increases >= MAX_INCREASES

	# Snapshot stats before training starts
	stat_snapshot_speed = GameData.race2_speed
	stat_snapshot_jump  = GameData.race2_jump_modifier
	stat_snapshot_level = GameData.run_stat_level
	stat_snapshot_t2    = GameData.training2_increases
	...
	if was_already_maxed:
		max_stat_notice.text = "Max Training 2 Reached!"
...
func _spawn_barrel() -> void:
	...
	var barrel_count = randi() % 3 + 1
	for i in barrel_count:
		var barrel = BARREL_SCENE.instantiate()
		var offset = randf_range(-300, 300)
		var random_x = panda.global_position.x + offset
		barrel.global_position = Vector2(
			random_x,
			panda.global_position.y - BARREL_SPAWN_HEIGHT
		)
		barrel.setup(0.0, 0.0)
...
func _check_stat_increase() -> void:
	if GameData.training2_increases >= MAX_INCREASES:
		return
	var next_threshold = SCORE_PER_INCREASE * (session_increases + 1)
	if score >= next_threshold:
		session_increases += 1
		GameData.training2_increases += 1
		GameData.apply_training2_stat()
		_update_stat_label()
		if GameData.training2_increases >= MAX_INCREASES:
			_on_max_reached()
...
func _on_player_died() -> void:
	...
	if was_already_maxed:
		...
		GameData.race2_speed         = stat_snapshot_speed
		GameData.race2_jump_modifier = stat_snapshot_jump
		GameData.run_stat_level      = stat_snapshot_level
		GameData.training2_increases = stat_snapshot_t2
...
func _update_stat_label() -> void:
	stat_label.text = "Stats +" + str(GameData.training2_increases) + "/5"
